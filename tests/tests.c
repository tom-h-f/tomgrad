...
void test_tensor_init_multiple_dims(void) {
    ...
    tg_err_t err = tensor_init(dims, 2, &tensor);
    TEST_ASSERT_EQUAL(SUCCESS, err);
...
void test_tensor_init_zero_dims_construction(void) {
    auto tensor = (tg_tensor_t*)NULL;
    tg_err_t err = tensor_init(NULL, 0, &tensor);
    TEST_ASSERT_EQUAL(SUCCESS, err);
    TEST_ASSERT_EQUAL(0, tensor->size);
    free(tensor->vals);
}
...
void test_tensor_scalar_add(void) {
    int dims[] = {3};
    auto tensor = (tg_tensor_t*)NULL;
    UNWRAP(tensor_init(dims, 1, &tensor));
    tensor->vals[0] = 1.0;
    tensor->vals[1] = 2.0;
    tensor->vals[2] = 3.0;
